...
sc = Scene()
camera_positions = [0, np.pi/2, np.pi, (3*np.pi)/4]
...
bounds = (2e5, 6e5) # These are the bounds for 
...
for i in sc.camera.iter_rotate(2*np.pi, 36, rot_center=ds.domain_center):
    ...
    if yt.is_root():
        sc.save(f'/gpfs/projects/CalderGroup/KianSpace/reu2023/plots/urca/3d_renders/rotate/rhoX(na23)/images/3d_render_{field[1]}_rot{i}.png', render=False, sigma_clip=2)
